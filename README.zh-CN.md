...
当前 Web GUI 的地图工作台提供快速火力任务/炮兵制图辅助：

- 地图对象列表会显示全部可用目标，不再只截取前几个对象。
- 对象列表支持三点菜单过滤，可按地面、空中、任务目标、出生点和其他类型筛选。
- 对象排序优先显示疑似小队队友，其后按当前射手或玩家位置从近到远排序。
- 每个地图对象都有独立的显示/隐藏开关。
- 火力任务只允许一个射手和一个目标处于启用状态。
- 可以在对象列表中直接把单位设置为射手或目标。
- 可以在地图上左键点击单位，弹出菜单后将该单位设置为射手或目标。
- 可以在地图空白位置左键点击，弹出攻击位置菜单后将该位置设置为目标；菜单支持 `Cancel` 取消。
- `Track POI` 可将目标绑定到 `point_of_interest` 并随 8111 数据刷新跟踪。
- 当前被设置为射手或目标的单位会在地图和对象列表中高亮显示。
- 地图符号使用阵营色底和深色内部线，提高在 War Thunder 地图背景上的可读性。
- 页面布局按地图和右侧任务面板整体居中，主要内容限制在视口高度内，避免默认缩放下出现整页纵向滚动。
