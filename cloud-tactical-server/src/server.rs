...
pub fn build_router(state: AppState) -> Router {
    let max_map_image_bytes = state.config.max_map_image_bytes;
...
async fn get_config(State(state): State<Arc<AppState>>) -> Json<serde_json::Value> {
    Json(serde_json::json!({
        ...
        "max_map_image_bytes": state.config.max_map_image_bytes,
    }))
}
...
async fn list_rooms(State(state): State<Arc<AppState>>) -> Json<Vec<serde_json::Value>> {
    let rooms = state.rooms.lock().await;
...
async fn get_room(
    ...
    Ok(Json(serde_json::json!({
        ...
        "map_generation": info.map_generation,
        "has_map_image": has_map_image,
...
async fn upload_room_map_image(
    ...
    if body.len() > state.config.max_map_image_bytes {
        return Err(AppError::new(
            StatusCode::PAYLOAD_TOO_LARGE,
            format!(
                "Map image exceeds {} byte limit",
                state.config.max_map_image_bytes
            ),
        ));
    }
    ...
    let uploaded_by =
        read_string_header(&headers, "x-wt8111-client-id").unwrap_or_else(|| "unknown".into());
    let password = read_string_header(&headers, "x-wt8111-room-password").unwrap_or_default();

    let mut rooms = state.rooms.lock().await;
    let room = rooms.get_room(&room_id).ok_or(RoomError::RoomNotFound)?;

    if room.info.password_hash.is_some() && !room.check_password(&password) {
        return Err(RoomError::InvalidPassword.into());
    }
